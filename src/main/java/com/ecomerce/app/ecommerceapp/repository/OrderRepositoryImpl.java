...
import com.ecomerce.app.ecommerceapp.exceptions.ResourceNotFoundException;
import com.ecomerce.app.ecommerceapp.model.NewOrder;
import com.ecomerce.app.ecommerceapp.model.Order;
...
@PersistenceContext
 ...
 private ItemService itemService;
    ...
    public OrderRepositoryImpl(EntityManager em, itemRepository itemRepo, CartRepository cRepo, OrderItemRepository oiRepo, ItemService itemService) {
        this.em = em;
 ...
 Optional<CartEntity>oCart=  cRepo.findByCustomerId(UUID.fromString(m.getCustomerId()));
     CartEntity cart = oCart.orElseThrow(() -> new
             ...
             String.format("Cart not  found for given customer (ID: %s)",
             m.getCustomerId())));
...
OrderEntity entity=(OrderEntity) em.createNativeQuery("  SELECT o.* FROM ecomm.orders o WHERE o.customer_id = ? AND   o.order_date >= ?  ", OrderEntity.class)
        ...
        .setParameter(2, OffsetDateTime.ofInstant(orderDate.toInstant(), ZoneId.of("Z")).truncatedTo(ChronoUnit.MICROS)).getSingleResult();
oiRepo.saveAll(cart.getItems().stream()
        .map(i -> new OrderItemEntity().setOrderId(entity.
                        getId())
